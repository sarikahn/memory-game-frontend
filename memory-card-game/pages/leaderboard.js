...
export default function Leaderboard() {
  ...
  useEffect(() => {
    fetch("https://memory-game-backend-4.onrender.com/scores")
      .then((res) => res.json())
      .then((data) => {
        // Sort scores by moves ascending
  ...
  return (
    <div className="p-8 min-h-screen bg-gray-100">
      <h1 className="text-3xl sm:text-4xl font-bold text-center mb-6">Leaderboard</h1>
          ...
          <tbody>
            {scores.map((score, index) => (
              <tr key={score._id} className="odd:bg-white even:bg-gray-100">
                <td className="border px-4 py-2">{index < 3 ? medals[index] : index + 1}</td>
